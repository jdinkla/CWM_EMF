...
12.01.2007 11 removed redirection
...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="Model:Reference">
		...
		<eStructuralFeatures xsi:type="ecore:EReference"
			...
			<xsl:variable name="class" select="ancestor::Model:Class"/> <!-- ../../@xmi.id"/>  --> <!-- this class -->
						...
						<xsl:for-each select="//Model:Class[@xmi.id=$classB]">
							...
							<xsl:if test=".//Model:Reference[@referencedEnd=$refB]">
								...
								<xsl:attribute name="eOpposite">
									<xsl:call-template name="get-q-name">
										<xsl:with-param name="package" select="$package" />
										<xsl:with-param name="xmi.id" select="$refB" />
									</xsl:call-template>
								</xsl:attribute>
							</xsl:if>
						</xsl:for-each>
	...
	<xsl:template match="XMI.content">
		<!-- xsl:variable name="filename" select="'../generated/models/cwm.ecore'" />
		<redirect:write select="$filename">  -->
			<ecore:EPackage xmi:version="2.0"
				xmlns:xmi="http://www.omg.org/XMI"
				xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
				xmlns:ecore="http://www.eclipse.org/emf/2002/Ecore"
				nsURI="http://www.dinkla.net/{@name}" name="CWM"
				nsPrefix="CWM">
				<!--  recurse -->
				<xsl:apply-templates />
				<!-- container object -->
				<eClassifiers xsi:type="ecore:EClass">
					<xsl:attribute name="name">
						<xsl:text>_CWM_Ecore_Container</xsl:text>
					</xsl:attribute>
	...
	<!-- function get-fq-name-eopposite -->
	<xsl:template name="get-fq-name-eopposite">
		<xsl:param name="package" />
		<xsl:param name="class" />
		<xsl:param name="x" />
		<xsl:param name="y" />

		<xsl:variable name="str">#//<xsl:value-of select="$x"/>/<xsl:value-of select="$y"/></xsl:variable>
	
		<xsl:for-each select="//*[@xmi.id=$class]">
			<xsl:variable name="mypackage" select="../../@name"/>
			<xsl:choose>
				<xsl:when test="$package = $mypackage"><xsl:value-of select="$str"/></xsl:when>
				<xsl:otherwise>ecore:EReference<xsl:text>&#32;</xsl:text>cwm_<xsl:value-of select="$mypackage"/>.ecore<xsl:value-of select="$str"/></xsl:otherwise>
			</xsl:choose>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template name="container-name">
		...
		<xsl:text>_</xsl:text>
		<xsl:value-of select="$name"/>
		<xsl:text>_Ecore_Container</xsl:text>
	</xsl:template>

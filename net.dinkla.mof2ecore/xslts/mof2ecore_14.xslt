...
28.09.2007 14 added renaming of elements named XML, also renamce nsUri and namespace of subpackages
...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="Model:Package">
		<eSubpackages>
			...
			<xsl:attribute name="nsURI"><xsl:value-of select="$name"/></xsl:attribute>
			<xsl:attribute name="nsPrefix"><xsl:value-of select="$name"/></xsl:attribute>
	...
	<xsl:template name="reformat-name">
		<xsl:param name="name" />
		<xsl:choose>
			<xsl:when test="$name = 'Instance'">CwmInstance</xsl:when>
			<xsl:when test="$name = 'Transformation'">CwmTransformation</xsl:when>
			<xsl:when test="$name = 'XML'">CwmXml</xsl:when>
